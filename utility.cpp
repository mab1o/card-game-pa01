...
// helper to remove, remove a node when case 1 or 2
...
// give the max value in the tree
CardsBST::Node* CardsBST::bstmax() const{
    Node* n = root;
    while(n->right){
        n = n->right;
    }
...
// give the min value in the tree
CardsBST::Node* CardsBST::bstmin() const{
    Node* n = root;
    while(n->left){
        n = n->left;
    }
...
// give a the card in position value for a tree in order
Card CardsBST::cardInPosition(int value) const{
    if (value > count()){ return Card('0',"0");}
...
// give a the card in position value for a tree in reverse order
Card CardsBST::cardInPositionReverse(int value) const{
    if (value > count()){ return Card('0',"0");}
